...
| Turn identity surface for Studio | **Resolved 2026-09-09** — `S1-SEAM-SPEC.md` §3–§5. Address (`path`/`ownerPath`) is captured at notify time and carried for reading; `subjectId` remains semantic identity; records are namespaced by `TreeId`. | Closed |
| `CausalEffect.path`/`ownerPath` undeclared | **Verified live kernel defect.** `toCausalEffect` (transactions.ts:1164) sets both on all four variants and casts `as CausalEffect`; `pending-rollback` reads them back through a double cast. Its comment records the bug this already caused — a field rollback wrote over its parent row. Fix: make both **required**; every live producer sets them. | **Open — fix now, independent of Studio** |
| `pending` turns | **Out of S1** (decided 2026-09-09). Own slice, S1P. S1 emits disposition `committed` only and must not manufacture `pending`/`discarded`. Schema must leave room for them without changing what net effects mean. | Scheduled — S1P |
| Dead causal-runtime chains | `transaction-capture-bridge`+`greenfield-transactions`, `confirmed-undo`+`reversal-planner`, `confirmed-redo`+`reapply-planner` have no non-spec importers. They carry spec files and shape assumptions about a capture path that is not live. | **Open — delete or revive before they mislead again** |
...
| S1 bundle delta (§8.5 #6) | **Measured. PASS.** bare tree +0 B; `transactions()` +17 B gzip against a ≤100 B budget; zero Studio symbols in production bundles. The first measurement FAILED at +161 B — the projection was a class method and could not tree-shake — and was fixed by moving it into `/internals`. Evidence: `smoke/S1-COST-MEASUREMENTS.md`. | Closed |
| S1 disabled overhead (§8.5 #7) | **PASS structurally**, pinned by `studio-unused-cost.spec.ts` plus zero bundle presence — code that is absent cannot cost runtime. The ≤1% *number* is reclassified as **performance characterization debt, not acceptance debt**: the harness noise floor is tens to hundreds of percent, so it cannot resolve a 1% effect either way. **Non-blocking for S1 and S2.** | Closed — see trigger below |
| ≤1% hot-path budget (deferred) | Returns the moment a slice puts work on the actual write path: an `if (observer)` branch per write, an event allocated before knowing anyone listens, or an index/history maintained while Studio is unattached. **S2 is the first real test**, being closer to the write path than committed-turn reading. Rerun a precise benchmark before shipping any such slice. | Open — conditional |
| Bounded confirmed history | `confirmedTurns` is append-only with no eviction. Pre-existing; S1 reads it rather than adding retention, so it is not an S1 regression. Studio makes the question easier to ask. | Open — kernel/history ownership |
