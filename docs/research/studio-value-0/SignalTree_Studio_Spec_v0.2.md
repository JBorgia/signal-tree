...
| S1 disabled overhead (§8.5 #7) | **Measured, no regression detectable — but the ≤1% threshold is NOT assertable with this harness.** Noise floor exceeds 1% (deltas range −33% to +75% on sub-millisecond work; S1 cannot make writes faster). The real guarantee is structural: no write-path code, and the projection is absent from the bundle entirely. | **Open — a 1% bound needs interleaved in-process A/B; recorded rather than rounded up** |
