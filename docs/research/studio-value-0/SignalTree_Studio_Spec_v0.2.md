...
| `CausalEffect.path` undeclared | Kernel defect found during the inventory: three modules (`reversal-planner`, `reapply-planner`, `pending-rollback`) read a `path` the interface does not declare, and producers disagree on whether it is set. Fix is wanted independent of Studio. | **Open — S1 acceptance item 5** |
...
| `pending` turns in S1? | Publishing pending turns exposes attempted work — the strongest causal story — but pending/rollback composition is the kernel's largest spec surface. May belong to S1 or to its own slice. | **Open — scope decision before implementation** |
