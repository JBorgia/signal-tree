...
| `packages/shared/src/`                                                                                       | Private build-time implementation           | Never published as a dependency. Rollup bundles required modules into public packages.                                                                   |
...
| `scripts/release.sh`, `scripts/publish-all.sh`, `scripts/ci-publish.sh`, `scripts/pre-publish-validation.sh` | Release operations                          | Direct publishing authority. All publish paths must validate the same built artifacts before `npm publish`.                                              |
