...
next change is a _memory_ change that could cost time on the hot path. Without a
...
size; @ngrx/signals is linear because it rebuilds or re-projects state
proportional to what it holds. At 1k SignalTree is ~6x @ngrx/signals; at 100k
the gap is a different kind of statement entirely.
...
SignalTree is flat to 100 consumers. @ngrx/signals is flat here too, but at a
level set by its collection-size term rather than by fan-out.
...
**SignalTree retains ~4x @ngrx/signals in the raw config.** That is the
...
In the featured config SignalTree retains active history. @ngrx/signals stays at
10 MB because it has no history primitive to configure — that row is not a win,
it is a missing feature.

## CORRECTION — the OOM footnote, and what it actually is

This section originally read: _"the history-recording update loop accumulates,
a previous sample's store is still reachable when the next is built, this is the
same residue Step 8 exists to attack."_ **Two of those three claims were wrong**,
