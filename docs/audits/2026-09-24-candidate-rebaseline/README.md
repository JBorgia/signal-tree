...
⚠️ **EARLIER VERSIONS OF THIS CHECKER PASSED THREE DELIBERATELY BROKEN
ADAPTERS**, each with zero violations and exit 0. Every hole is now a named
check, and — more importantly — the adapters are EXECUTABLE and enforced:

    1  only the FINAL state was compared, so an adapter that destroyed P2's
       y=2 while rejecting P1 and repaired it while accepting P2 scored clean
    2  "other authority preserved" tested `typeof === 'boolean'`, which `false`
       satisfies, and never reached the verdict
    ...
    4  `already-settled` on a freshly opened handle fell into an else-branch
       requiring nothing; an exception from the SECOND settlement was swallowed
    5  publication was bounded around the FIRST settlement only, so an adapter
...
adapters and requires each verdict:
    ...
    control   settles correctly            must PASS   0 violations
    midstep   corrupts between steps       must FAIL   6 violations
    already   already-settled on a new
              handle, doing nothing        must FAIL   4 violations
    torn2     torn publication during the
              SECOND settlement            must FAIL   2 violations

The control is there so the suite cannot be satisfied by a checker that rejects
everything. Proven load-bearing: disabling the second-boundary check turns the
self-test red with *"checker did NOT catch torn2"*.
