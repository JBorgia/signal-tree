...
SignalTree 15.0 keeps the app-facing primitives on the core barrel: build state with `signalTree`, `entityMap`, `stored`, `asyncSource`, `asyncQuery`, `loader`, `compared`, `linked`, and the built-in enhancers. Custom marker registration and enhancer-author internals are no longer published as an extension SDK.
