...
const DISPOSITION = {
  ...
  core: [
    'REBUILD',
    'the v15 kernel itself; becomes the new-generation kernel package',
  ],

  // ── ABSORB: the semantic job now lives inside an earned v15 boundary ─────
  batching: [
    'ABSORB',
    'now core/enhancers/batching; `batching()` is public (BATCHING-OWNERSHIP-0 owes only its representation)',
  ],
  ...
  'time-travel': [
    'ABSORB',
    'now core/enhancers/restoration; `restoration()`/`undoable()` are public',
  ],
  entities: [
    'ABSORB',
    'now the core `entityMap` marker and EntitySignal (ENTITY-REPRESENTATION-OWNERSHIP-0 owes the representation)',
  ],
  types: [
    'ABSORB',
    'type surface folded into core/lib/types.ts and the owner modules (TYPE-BARREL-CONVERGENCE-0)',
  ],
  ...
  'callable-syntax': [
    'DELETE',
    'deleting commit: "the transform can never run"',
  ],
  'syntax-transform': [
    'DELETE',
    'same defect as callable-syntax — a build transform that cannot run',
  ],
  memoization: [
    'DELETE',
  ...
  enterprise: [
    'DELETE',
    'deleting commit: "not published in 14.0.0" — never reached the 14 generation',
  ],
  middleware: [
    'DELETE',
    'superseded by the enhancer contract; no surviving public job',
  ],
  presets: [
    'DELETE',
    'configuration bundles over an enhancer set that no longer exists',
  ],
  ...
  guardrails: [
    'LEGACY-ONLY',
    'shipped 14.1.1 against the v14 kernel; its interception model predates the v15 observation architecture',
  ],
  ...
  events: [
    'LEGACY-ONLY',
    'published 14.1.3 and still installable; old implementation deleted by EVT-DEL. A future event/notification package is a CANDIDATE, not an earned rebuild',
  ],
  'ng-forms': [
    'LEGACY-ONLY',
    'published 14.1.3 and still installable; old implementation deleted by NGF-DEL. Angular forms integration is a semantic job candidate; the package boundary is NOT earned',
  ],

  // ── private, never published ─────────────────────────────────────────────
  shared: [
    'ABSORB',
    'private workspace package; kernel utilities consumed by core and reachable from the bare bundle',
  ],
};
...
const present = sh(`ls -1 packages`)
  .split('\n')
  .filter(Boolean)
...
for (const dir of all) {
  ...
  if (live) {
    ...
    version = j.version;
    name = j.name;
    priv = Boolean(j.private);
  } else {
    const sha = sh(
      `git log --all --diff-filter=D --format=%H -1 -- packages/${dir}/package.json`
    );
    ...
    try {
      ...
      /* unparseable historical manifest */
    }
  }
  ...
  rows.push({
    ...
    coreRange: PUBLISHED_CORE_RANGE[dir],
  });
}
...
console.log(
  `PACKAGE-GENERATION-CENSUS-0 — ${rows.length} package(s) in the denominator\n`
);
console.log(
  'package                      published    in-repo      disposition'
);
for (const r of rows.sort(
  ...
  console.log(
    `  ${r.name.padEnd(28)}${pub.padEnd(13)}${String(r.version).padEnd(13)}${
      r.disposition ?? '‼ NONE'
    }`
  );
}
console.log('\nby disposition:');
for (const [d, rs] of Object.entries(byDisp).sort())
  console.log(`  ${String(rs.length).padStart(3)}  ${d}`);
...
console.log(
  'NEVER PUBLISHED       : ' +
    (rows.length - published.length) +
    ' — no users to strand'
);
console.log('\nWHAT THE REGISTRY WOULD ACTUALLY ADMIT:');
for (const r of published)
  if (r.coreRange) console.log(`  ${r.name.padEnd(28)} core ${r.coreRange}`);
console.log(`
