...
function classify(src) {
  ...
  ts.forEachChild(sf, (n) => {
    if (
      ...
      n.moduleSpecifier.getText(sf).includes('@angular/core')
    ) {
      const b = n.importClause?.namedBindings;
      if (b && ts.isNamedImports(b))
        for (const el of b.elements) imported.add(el.name.text);
    }
  });
  const value = new Set(),
    type = new Set();
  const visit = (n, inType) => {
    ...
    if (ts.isIdentifier(n) && imported.has(n.text))
      (nowType ? type : value).add(n.text);
    ts.forEachChild(n, (c) => visit(c, nowType));
  };
  visit(sf, false);
  return {
    value: [...value].sort(),
    type: [...type].filter((x) => !value.has(x)).sort(),
  };
}
...
function classifyByStatementKind(src) {
  ...
  ts.forEachChild(sf, (n) => {
    if (
      ...
      const b = n.importClause?.namedBindings;
      if (b && ts.isNamedImports(b))
        for (const el of b.elements) symbols.push(el.name.text);
    }
  });
...
check(
  'planted VALUE symbol is runtime coupling',
  r.value.includes('signal'),
  `value: ${r.value.join(',')}`
);
check(
  'planted TYPE symbol is NOT runtime coupling',
  !r.value.includes('Signal'),
  `type: ${r.type.join(',')}`
);
check(
  'second TYPE symbol is NOT runtime coupling',
  !r.value.includes('WritableSignal')
);
check(
  'type symbols are still reported as type coupling',
  r.type.includes('Signal') && r.type.includes('WritableSignal'),
  r.type.join(',')
);
...
check(
  'statement-kind rule WOULD misclassify the types as runtime',
  m.runtime === true && m.symbols.includes('Signal'),
  'it marks the whole statement runtime, dragging Signal/WritableSignal in'
);

// and the real kernel census must remain free of Angular coupling
const ROOT = new URL('..', import.meta.url).pathname.replace(/\/$/, '');
try {
  const census = JSON.parse(
    (await import('node:child_process')).execSync(
      `node ${ROOT}/tools/angular-service-census.mjs --json`,
      { encoding: 'utf8' }
    )
  );
  const coupled = Object.keys(census);
  check(
    'real run: kernel has no Angular-coupled modules',
    coupled.length === 0,
    `modules: [${coupled.join(',')}]`
  );
} catch (e) {
...
if (failures) {
  console.error(`\n❌ angular-coupling control: ${failures} failure(s).`);
  process.exit(1);
}
console.log(
