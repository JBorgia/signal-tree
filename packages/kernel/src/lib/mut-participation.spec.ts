...
describe('MUT-2 — does surviving machinery carry the AUTHORED vs REALIZED distinction?', () => {
   ...
   * `WriteAttribution.participation` carries it (that subset type was deleted
   * in 15.0 with MutationEnvelope; the field lives on `WriteMetadata`). R3 showed the notifier's PATH
    ...
    expect(seen).toEqual([
      {
        ...
        meta: {
          ...
          // ⚠️ `positionIds: [3]` STOOD HERE until 15.0 deleted
          // `WriteMetadata.positionIds` (SUBJECT-IDENTITY-OWNERSHIP-0) — eight
          // writers, no production reader, and the owner position reaches every
          // observer authoritatively through the `notify(...)` parameter and the
          // node's own `__positionIds`. `subjectIds` remains in the shape (here
          // undefined, since a scalar undo names no subject) because on the
          // interceptor replay route it is the ONLY carrier of the effect's
          // subject — see its note in mutation-types.ts.
