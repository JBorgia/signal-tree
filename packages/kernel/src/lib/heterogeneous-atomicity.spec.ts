...
describe('heterogeneous atomicity: scalar + structural in one transaction', () => {
  it('spans two private substrate commits, which is permitted and not a transaction identity', async () => {
    ...
    resetPathNotifier();
    // ⚠️ `getPathNotifier().setBatchingEnabled(false)` USED TO SIT HERE AND WAS
    // INERT. Tree construction called `setBatchingEnabled(config.batchUpdates
    // !== false)` on the same global notifier moments later, silently restoring
    // batching — so these cases always ran BATCHED while appearing to ask for
    // synchronous delivery. Retiring `batchUpdates` (BATCH-UPDATES-INTENT-0)
    // removed the overwrite and exposed it. Removed rather than honoured,
