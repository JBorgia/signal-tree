...
function createScalarHarness(size: number): ScalarHarness {
  ...
  }) as unknown as ISignalTree<Record<string, ScalarLeaf>>;
  ...
  const targetLeaf = tree.$[targetKey] as unknown as ScalarLeaf;
...
function createEntityHarness(size: number): EntityHarness {
  const tree = signalTree(
    ...
    { capabilities: ['causal-runtime'] }
  ) as unknown as ISignalTree<{
    rows: EntityCollection;
  }>;
  // Scaffolding cast: this harness measures THROUGHPUT, not typing. Widened
  // because whole-value NodeAccessor no longer overlaps the method-bag shape.
  const rows = tree.$.rows as unknown as EntityCollection;
