 ...
 * `tsc -p packages/kernel/tsconfig.typecheck.json`, EXCLUDED from vitest (the
...
export const _branchRead: { name: string; age: number; address: { city: string } } =
  rootTree.$.user();
rootTree.$.user({ name: 'Ada', age: 44, address: { city: 'Boise' } });
...
export const _builtBranchRead: { name: string; age: number; address: { city: string } } =
  built.$.user();
built.$.user({ name: 'Ada', age: 44, address: { city: 'Boise' } });
