...
export function getOwnedSubjectIds(node: unknown): number[] | undefined {
  if (isTraversableNode(node)) {
    ...
    // ⚠️ NO SIDECAR FALLBACK HERE, unlike every sibling reader above.
    //
    // The sidecar slot for subject ids lost its only writer in 15.0 when
    // `defineOwnedSubjectIds` was deleted (MUTATION-ENVELOPE-OWNERSHIP-0) — it
    // ran only under `if (options.subjectIds)`, and no caller ever supplied
    // them. Subject ids arrive exclusively as a DIRECT `__subjectIds` property
    // written by `entity-signal.ts`, which never selects sidecar storage.
    //
    // Reading a slot nothing can write is not defensive, it is a claim that a
    // second storage mode exists for this fact. It does not.
    return undefined;
  }
