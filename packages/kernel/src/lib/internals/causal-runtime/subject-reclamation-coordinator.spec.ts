...
describe('subject reclamation coordinator', () => {
  ...
  it('reclaims an eligible tombstoned subject without publication or reactive churn', () => {
    ...
    expect(observedName()).toBe('absent');
    // `runs` counts OBSERVER EVALUATIONS, which measures memoization, not a
    // semantic law. The neutral kernel derivation recomputes on read by design,
    // so this count is carrier-dependent and belongs to the Angular package,
    // where `computed()` guarantees memoization. The invariant this suite is
    // named for — no publication churn — is the `notify` assertion below, and
    // it holds under both carriers. What the kernel must guarantee is that the
    // observer stays CORRECT across reclamation, asserted above.
