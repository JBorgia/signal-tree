...
describe('EntityNode field writes (Option B+ computed-based shim)', () => {
  ...
  it('field property is a readable cell', () => {
    const api = makeApi();
...
describe('entity subject physical inventory', () => {
  ...
  it('proves terminal held facades do not depend on retained value backing once the subject is tombstoned', () => {
    ...
    expect(rowRuns).toBeGreaterThanOrEqual(2); // memoization count -> Angular pkg
    expect(nameRuns).toBeGreaterThanOrEqual(2); // memoization count -> Angular pkg
    ...
    expect(observedRow()).toBe('Alice');
    expect(observedName()).toBe('Alice');
    expect(rowRuns).toBeGreaterThanOrEqual(3); // memoization count -> Angular pkg
    expect(nameRuns).toBeGreaterThanOrEqual(3); // memoization count -> Angular pkg
  });
