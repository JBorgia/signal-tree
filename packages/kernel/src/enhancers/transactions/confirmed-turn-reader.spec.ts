...
describe('confirmedTurnReader', () => {
  ...
  it('2. every effect preserves path and ownerPath', () => {
    ...
    const effects =
      confirmedTurnReader(tree as never)?.readConfirmedTurns().turns[0]
        ?.effects ?? [];
  ...
  it('5. two trees with identical numeric ids stay isolated', () => {
    const a = cartTree();
    const b = cartTree();
    a.transact(() => a.$['total'](1)).confirm();
    b.transact(() => b.$['total'](2)).confirm();
  ...
  it('7. the reader cannot mutate retained history', () => {
    ...
    (first?.turns as unknown as unknown[])?.push({
      id: 999,
      positions: [],
      effects: [],
    });
...
describe('destroyed-tree lifecycle (acceptance 9)', () => {
  ...
  it('distinguishes no-transactions, live-but-empty, and destroyed', () => {
    ...
    const idle = cartTree();
    const idleSnapshot = confirmedTurnReader(
      idle as never
    )?.readConfirmedTurns();
    ...
    const used = cartTree();
    used.transact(() => used.$['total'](1)).confirm();
