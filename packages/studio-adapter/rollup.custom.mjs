...
export default (config, options) => {
  const base = baseConfigFactory(config, options);
  return {
    ...base,
    // Preserve the package boundary: Studio must observe the application's kernel.
    external: (id) =>
      id.startsWith('@signal-tree/') ||
      id === 'tslib' ||
      id.startsWith('node:'),
    plugins: [
      ...base.plugins.filter(
        (plugin) =>
          plugin?.name !== 'typescript' && plugin?.name !== 'dts-bundle'
      ),
      // Nx substitutes dependency output roots for paths, losing package subpaths.
      typescript({
        tsconfig: path.join(packageRoot, 'tsconfig.lib.prod.json'),
        declarationDir: path.join(
          packageRoot,
          '../../dist/packages/studio-adapter/src'
        ),
