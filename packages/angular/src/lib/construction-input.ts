...
export function prepareConstructionInput(initialState: object): object {
  ...
  const visit = (value: unknown, path: string): unknown => {
    if (isSignal(value)) {
      throw new TypeError(
        `SignalTree: Angular signal at ${path}. Pass an initial value or leaf(untracked(existing)) for an independent snapshot. Use leaf(existing) only to store the signal as data; its writes remain external to the tree.`
      );
    }
