 ...
 *   2. importing `entityMap` without `restoration` pulls in no history engine.
 *
...
const CASES = [
  ...
  {
    name: 'entityMap() without restoration() — no history engine code',
    code: `import { signalTree, entityMap } from '${KERNEL}';\nconst t = signalTree({ rows: entityMap({ selectId: (row) => row.id }) });\nt.$.rows.addOne({ id: 1 });\nglobalThis.__s = t.$.rows.byId(1)?.();\n`,
    forbidden: ['canRedo', 'canUndo', 'restorationManager'],
  },
];
