...
provenance. Schema 5 also records the selected measured and warmup rounds, three
independent recurring-workload normalizations, observed cohort positions, and
the sourced typed DX, speed, density, and causal-foundation ledger. Construction
has no public rank or value score. The exporter refuses development-mode timings unless
`ALLOW_DEV_BENCHMARKS=1` is set for diagnostic output; set
`BENCHMARK_MODE=steady` for the 100-round plan. The browser defaults to 25
measured rounds and accepts a custom count from 1 through 1,000.
